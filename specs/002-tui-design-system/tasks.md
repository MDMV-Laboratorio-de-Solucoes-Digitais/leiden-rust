...
- [ ] T001 Verify `rust-toolchain.toml` pins stable toolchain, edition 2024, MSRV floor 1.88.0 (Constitution §VII, plan.md §Constraints)
- [ ] T002 [P] Verify `[workspace.lints]` in workspace `Cargo.toml` enforces `unsafe_code = deny`, `missing_docs = deny`, `missing_debug_implementations`, `panic`/`unwrap_used`/`expect_used` denied, `clippy::pedantic = deny` (Constitution §II, guide-to-strict-rust.md)
- [ ] T003 Create `pub mod colors;` and `pub mod styles;` declarations in `leiden/crates/leiden-tui/src/ui/mod.rs`
...
- [ ] T004 [P] Write failing tests: all 26 color constants compile as `const` items (`const _: Color = BG_0;` pattern from quickstart §2) in `leiden/crates/leiden-tui/src/ui/colors.rs`
- [ ] T005 Implement color constants in `leiden/crates/leiden-tui/src/ui/colors.rs`: `BG_0`–`BG_4`, `FG_0`–`FG_3`, `ACCENT_*` (5), `COMMUNITY_COLORS: [Color; 12]` — all `pub const` with `///` docs (FR-001, data-model §1, design-system.md §9)
- [ ] T006 Write failing test: ANSI fallback completeness — every `*_ANSI` constant compiles and `COMMUNITY_COLORS_ANSI.len() == 6` (quickstart §11) in `leiden/crates/leiden-tui/src/ui/colors.rs`
- [ ] T007 Add ANSI fallback constants to `colors.rs`: `BG_*_ANSI`, `FG_*_ANSI`, `ACCENT_*_ANSI`, `COMMUNITY_COLORS_ANSI: [Color; 6]` (FR-014, contracts §1.2, design-system.md §10.1)
- [ ] T008 Write failing tests: `community_color()` determinism + wrap at 12, `supports_truecolor()` detection for COLORTERM/TERM/conservative default (quickstart §4, §5) in `leiden/crates/leiden-tui/src/ui/colors.rs`
- [ ] T009 Implement `community_color(community_id: u32) -> Color` and `supports_truecolor() -> bool` in `leiden/crates/leiden-tui/src/ui/colors.rs` (FR-007, FR-013, data-model §1.5)
- [ ] T010 [P] Write contrast ratio validation test computing WCAG ratios for all documented pairs (data-model §6, design-system.md §2.3) in `leiden/crates/leiden-tui/src/ui/colors.rs`
- [ ] T046 [P] Write failing test: `resolve_color()` selects RGB when `supports_truecolor()` returns true, ANSI fallback when false; verifies all 15 color pairs have a corresponding `_ANSI` constant (data-model §2.1, contracts §1.4) in `leiden/crates/leiden-tui/src/ui/colors.rs`
- [ ] T047 Implement `resolve_color(color: Color, ansi: Color) -> Color` in `leiden/crates/leiden-tui/src/ui/colors.rs`, gating on `supports_truecolor()` (FR-013/FR-014, B1)
...
- [ ] T011 [US1] Write failing tests: `state_theme_covers_all_variants` (all 4 states return non-empty color/symbol/label) + `state_indicators_are_unique` (quickstart §6, contracts §2.2)
- [ ] T012 [US1] Implement `state_color()`, `state_indicator()`, `state_label()` + `INDICATOR_*` symbol constants in `leiden/crates/leiden-tui/src/ui/styles.rs` (FR-003, data-model §§4.3, 5)
- [ ] T013 [US1] Update `leiden/crates/leiden-tui/src/ui/status_bar.rs` to render state indicator symbol + label using state theme functions, with progress gauge placeholder for Running
- [ ] T014 [US1] Write snapshot test: status bar renders distinct `○`/`●`/`✓`/`✗` + label per state using `TestBackend` (quickstart §10)
...
- [ ] T048 Write failing test: `help_overlay()` returns a `Block` with `BG_1` background and `ACCENT_PRIMARY` `BorderType::Rounded` border; key bindings are grouped into labeled categories (quickstart §10, FR-017, design-system.md §5.5)
- [ ] T049 Implement `help_overlay()` widget in `leiden/crates/leiden-tui/src/ui/mod.rs` (or new `src/ui/help.rs`) with grouped key bindings table, `BG_1` background, `ACCENT_PRIMARY` border (FR-017, data-model §8)
- [ ] T050 Write snapshot test: help overlay renders over all panels with `?`, dismisses on any key, preserves panel state (TestBackend, SC-001, SC-009)
...
- [ ] T015 [US2] Write failing tests: `selected_row_style()` uses `BG_3` + `FG_0` + `BOLD` (not `REVERSED`); `no_italic_in_style_presets` covers all presets (quickstart §8, FR-015)
- [ ] T016 [US2] Implement `header_style()`, `selected_row_style()`, `normal_row_style()` as `const fn` in `leiden/crates/leiden-tui/src/ui/styles.rs` (FR-015, data-model §3.2)
- [ ] T017 [US2] Update `leiden/crates/leiden-tui/src/ui/community.rs` to use `community_color()` for color blocks and `selected_row_style()` for row selection (US-2, design-system.md §5.1)
- [ ] T018 [US2] Update `leiden/crates/leiden-tui/src/ui/graph.rs` to use `community_color()` for node circle colors (US-2, design-system.md §5.2)
- [ ] T019 [P] [US2] Write snapshot test: verifying community color block matches graph node color for the same community ID (TestBackend)
...
- [ ] T020 [US3] Write failing test: `layout_mode()` returns correct `LayoutMode` at all breakpoints and interior values (quickstart §7, data-model §4.2)
- [ ] T021 [US3] Implement `LayoutMode` enum + `layout_mode(width: u16) -> LayoutMode` as `const fn` in `leiden/crates/leiden-tui/src/ui/styles.rs` (FR-004, data-model §4)
- [ ] T022 [US3] Implement responsive layout engine in `leiden/crates/leiden-tui/src/ui/mod.rs` dispatching on `LayoutMode`, plus panel toggle redistribution per the toggle matrix (FR-004, FR-005, design-system.md §§3.3–3.5)
- [ ] T023 [US3] Write snapshot tests: all 4 breakpoints + all `g`/`l` toggle combinations render correctly (TestBackend, SC-003)
...
- [ ] T024 [US4] Write failing tests: each `log_*_style()` returns correct color + modifier, no `ITALIC` (quickstart §8, FR-008)
- [ ] T025 [US4] Implement `log_error_style()` through `log_trace_style()` as `const fn` in `leiden/crates/leiden-tui/src/ui/styles.rs` (FR-008, data-model §4.4)
- [ ] T026 [US4] Update `leiden/crates/leiden-tui/src/ui/log_pane.rs` to apply severity styles to `[LEVEL]` prefixes and implement 500-entry FIFO ring buffer (US-4, design-system.md §5.3)
- [ ] T027 [US4] Write snapshot test: log pane renders `[ERROR]` = bold red, `[WARN]` = bold amber, `[INFO]` = teal, `[DEBUG]` = FG_2, `[TRACE]` = FG_3 dim (TestBackend)
...
- [ ] T028 [US5] Write failing tests: `focused_border_style()` = `ACCENT_PRIMARY`, `unfocused_border_style()` = `FG_3`, `key_hint_style()` = `FG_3` + `DIM` (quickstart §8, FR-006)
- [ ] T029 [US5] Implement `focused_border_style()`, `unfocused_border_style()`, `title_style_focused()`, `title_style_unfocused()` as `const fn` in `leiden/crates/leiden-tui/src/ui/styles.rs` (FR-006, data-model §3.1)
- [ ] T030 [P] [US5] Implement `key_hint_style()`, `key_letter_style()` as `const fn` in `leiden/crates/leiden-tui/src/ui/styles.rs` (data-model §3.3)
- [ ] T031 [US5] Implement `focused_block()`, `unfocused_block()`, `panel_block()` in `leiden/crates/leiden-tui/src/ui/styles.rs` using `BorderType::Rounded` (FR-010, data-model §4.5)
- [ ] T032 [US5] Wire `Tab` focus cycle with hidden-panel skipping and single-panel no-op in `leiden/crates/leiden-tui/src/event.rs` + `leiden/crates/leiden-tui/src/ui/mod.rs` (FR-006, US-5)
- [ ] T033 [US5] Write snapshot test: `Tab` changes border color to `ACCENT_PRIMARY`; hidden panel skipped; `Tab` no-op when single panel visible (TestBackend)
...
- [ ] T034 [US6] Run contrast ratio validation for all documented pairs — `FG_0`/`FG_1`/`FG_2`/`ACCENT_*` on `BG_0`/`BG_3` (data-model §6, design-system.md §2.3, FR-012, SC-004)
- [ ] T035 [US6] Run `supports_truecolor()` + ANSI fallback completeness tests in 16-color mode (quickstart §5, §11)
- [ ] T036 [US6] Write integration test: forcing `COLORTERM` unset + `TERM=xterm`, verify state indicators (`○`/`●`/`✓`/`✗`) remain distinguishable by symbol + color at 16-color depth (US-6 scenario 3, SC-005)
...
- [ ] T037 [US7] Write failing snapshot test: Running state status bar shows progress gauge `5/10` + quality value `Q=0.4231` (quickstart §10, FR-009)
- [ ] T038 [US7] Implement progress gauge rendering in `leiden/crates/leiden-tui/src/ui/status_bar.rs` using `BG_4` (empty) + `ACCENT_INFO` (fill) with ratio `iteration / iteration_cap` (FR-009, design-system.md §7.1)
- [ ] T039 [US7] Implement quality sparkline with 20-iteration fixed window + auto-scaling to observed range in `leiden/crates/leiden-tui/src/ui/status_bar.rs` (US-7, design-system.md §7.2)
- [ ] T040 [US7] Write test: sparkline shows only 20 most recent iterations (oldest scroll off left); ΔQ < 0 renders in `ACCENT_ERROR` with negative sign; γ to 2 decimal places (US-7 scenarios 2–4, FR-011)
- [ ] T051 Write test: all FR-011 number formats verified — modularity `Q=0.4231` (4dp), `ΔQ=+0.0033` (4dp signed), node/edge counts with thousands separators (e.g. `12,345`), iteration `current/cap`, `γ=1.50` (2dp) (FR-011, SC-008, design-system.md §4.2)
...
- [ ] T041 [P] Run lint compliance gate: `cargo clippy -p leiden-tui --all-targets -- -D warnings` + `cargo doc -p leiden-tui --no-deps` (fails on missing_docs) (Constitution §II, §IV, quickstart §9)
- [ ] T042 Run full workspace test suite: `cargo test --workspace` + `cargo nextest run`
- [ ] T043 Run all quickstart.md validation scenarios (§2–§11) end-to-end and confirm pass
- [ ] T044 [P] Verify all 14 const fn style presets + 10 Unicode symbol constants + 15 color constants + 15 ANSI fallback constants + 3 non-const state theme fns + 1 LayoutMode enum + 1 layout_mode fn + 3 block builder fns + 1 resolve_color fn match `contracts/design-system-api.md` API surface exactly (FR-002 through FR-017, resolve_color invariant)
- [ ] T045 Run `cargo fmt --check` on the `leiden-tui` crate
- [ ] T052 Write test: render loop completes each frame in under 50 ms, including channel drain and widget rendering, at 20 FPS poll rate (SC-007, design-system.md §7)
- [ ] T053 [P] Run performance contracts under `--release`: `ct --workspace --release` to exercise `#[cfg(not(debug_assertions))]`-gated perf tests (Constitution §DevWorkflow, SC-007)
- [ ] T054 Write test: no emoji characters appear in rendered TUI output; all symbols drawn from documented Unicode BMP set (SC-009, FR-016, design-system.md §4.3)
- [ ] T055 Write test: partitions with ≤100 communities render without scrolling; >100 communities scroll with deterministic color cycling (SC-010, FR-018)
- [ ] T056 Run `cargo deny check` for dependency advisories and licenses (Constitution §DevWorkflow CI pipeline)
