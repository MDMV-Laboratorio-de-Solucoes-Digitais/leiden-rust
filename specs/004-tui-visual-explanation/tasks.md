...
- [X] T041 Wire `ExplanationState::from_leiden_event` and `ExplanationState::completed` into `App::push` so the explanation panel updates on every Leiden event and shows the completion summary on `Terminated` per FR-004, US2/AC1, US3/AC1 (partial)
- [X] T042 Implement phase-level vs micro-step stepping semantics so the `t` granularity toggle changes actual stepping behavior (currently only flips the status-bar badge) per FR-005, US2/AC2 (partial)
- [X] T043 Remove or wire the unused `AppAction`/`map_key_event` key-mapping abstraction and unused `explanation::Phase` enum to avoid divergence from `App::handle_key` per plan: event loop (unrequested)

---

## Phase 8: Convergence

- [X] T044 [CRITICAL] Remove `release_max_level_warn` feature from tracing dependency in `leiden/crates/leiden-tui/Cargo.toml` so `cargo test --workspace --release` passes without compiling out test tracing events per Constitution V (--release test gate) (contradicts)
- [X] T045 Format all test files in `leiden/crates/leiden-tui/tests/` to satisfy `cargo fmt --check` per Constitution Development Workflow (CI pipeline) (contradicts)
