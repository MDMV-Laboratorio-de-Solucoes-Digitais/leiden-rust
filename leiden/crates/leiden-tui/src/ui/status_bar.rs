...
pub fn render_status_bar(frame: &mut Frame<'_>, app: &App, area: Rect) {
    ...
    let playback_span = if app.playback.is_playing {
        Span::styled(
            " ▶ Playing",
            Style::new().fg(ACCENT_INFO).add_modifier(Modifier::BOLD),
        )
    } else {
    ...
    let ratio = f64::from(app.iterations) / f64::from(app.params.iteration_cap.max(1));
    ...
    let mode_span = match app.playback.granularity {
        GranularityMode::PhaseLevel => Span::styled("  Mode: Phase", Style::new().fg(FG_1)),
        GranularityMode::MicroStep => Span::styled("  Mode: Micro", Style::new().fg(ACCENT_INFO)),
    };
