...
fn deterministic_rng(seed: u64) -> impl FnMut() -> f64 {
    ...
    let mut state = seed.wrapping_mul(LCG_MULTIPLIER).wrapping_add(LCG_INCREMENT);
    move || {
...
fn seed_position(index: usize, total: usize) -> Point2D {
    ...
    Point2D::new(x.clamp(VIRTUAL_MIN, VIRTUAL_MAX), y.clamp(VIRTUAL_MIN, VIRTUAL_MAX))
}
...
impl ForceSimulation {
    ...
    pub fn tick(&mut self, partition: &[(String, u32)], edges: &[(String, String)]) {
        ...
        for (&comm_id, positions) in &comm_positions {
            if !positions.is_empty() {
                ...
                let _ = self.community_centroids.insert(
                    comm_id,
        ...
        for (node_id, delta) in &deltas {
            if let Some(pos) = self.node_positions.get_mut(node_id) {
                *pos = pos
                    .add_scaled(*delta, 1.0)
                    .clamp(VIRTUAL_MIN, VIRTUAL_MAX, VIRTUAL_MIN, VIRTUAL_MAX);
            }
        }
    }
    ...
    pub fn reset(&mut self, nodes: &[String]) {
        ...
        for (i, node) in nodes.iter().enumerate() {
            ...
            let _ = self.node_velocities.insert(node.clone(), Point2D::new(0.0, 0.0));
        }
        self.community_centroids.clear();
    }
}
