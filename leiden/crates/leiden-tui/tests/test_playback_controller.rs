...
fn default_controller_is_paused_phase_level() {
    ...
    assert!(
        !defaulted.step_requested,
        "default() must not have a step pending"
    );
    assert_eq!(
...
fn request_step_auto_pauses() {
    ...
    assert!(
        ...
        "playback must be running before the step"
    );
    ...
    assert!(
        controller.step_requested,
        "request_step must set step_requested"
    );
}
...
fn on_preset_switch_pauses_and_preserves_granularity() {
    ...
    assert!(
        controller.is_playing,
        "playback must be running before switch"
    );
    ...
    assert!(
        !controller.step_requested,
        "preset switch must clear step_requested"
    );
    assert_eq!(
        controller.granularity,
        GranularityMode::MicroStep,
...
fn n_key_steps_and_auto_pauses() {
    ...
    assert!(
        ...
        "Space must start playback before stepping"
    );
...
fn right_arrow_key_steps() {
    ...
    assert!(
        !app.playback.is_playing,
        "Right arrow must keep playback paused"
    );
}
...
fn preset_key_preserves_granularity_and_pauses() {
    ...
    assert!(
        app.playback.is_playing,
        "Space must start playback before the switch"
    );

    app.handle_key(KeyEvent::from(KeyCode::Char('2')));
    assert!(
        !app.playback.is_playing,
        "preset key must auto-pause playback"
    );
    assert_eq!(
        app.playback.granularity,
        GranularityMode::MicroStep,
