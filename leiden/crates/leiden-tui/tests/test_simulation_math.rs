...
fn simulation_screen_coordinates_maps_to_area() {
    ...
    for (node, (x, y)) in &coords {
        ...
        assert!(*x >= 0.0 && *x <= f64::from(area.width) + 1.0, "x={x} out of area");
        assert!(*y >= 0.0 && *y <= f64::from(area.height) + 1.0, "y={y} out of area");
    }
}
...
fn simulation_reset_restores_initial_positions() {
    ...
    for node in &nodes {
        let reset_pos = sim.node_positions.get(node).expect("node exists after reset");
