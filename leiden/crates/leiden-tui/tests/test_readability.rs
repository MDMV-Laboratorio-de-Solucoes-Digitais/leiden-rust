...
fn flesch_kincaid_complex_text_higher_grade() {
    ...
    assert!(complex > simple, "Complex text ({complex}) should score higher than simple ({simple})");
}
...
fn wrapped_lines_respect_max_width() {
    ...
    assert!(lines.len() <= 3, "Should have at most 3 lines, got {}", lines.len());
    for line in &lines {
        assert!(
...
fn wrapped_lines_max_three() {
    let state = ExplanationState {
        headline: String::new(),
        analogy_text: "This is a very long sentence that contains many many words and will definitely \
