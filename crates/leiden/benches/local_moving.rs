//! Criterion benchmark: Local moving pass on `lfr_small.edg` (T121).
...
fn bench_local_moving(c: &mut Criterion) {
    ...
    let _ = c.bench_function("local_moving_lfr_small", |b| {
        b.iter(|| {
            let res = Leiden::new()
...
criterion_group!(benches, bench_local_moving);
